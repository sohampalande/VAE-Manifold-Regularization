...
if __name__ == '__main__':
    ...
    model_name = 'vae_model_2' #change name of model

    path = './MNIST_results/' + directory + '/models/' +  model_name 

    vae_model_2 = VariationalAutoencoder(latent_dim)
    vae_model_2 = vae_model_2.train(trainloader, epochs=epochs)

    torch.save(vae_model_2, path)

    ########### Visualize ###############

    viz.plot_latent(vae_model_2, trainloader)  #latent space
    viz.plot_reconstructions_space(vae_model_2)  #latent space reconstructions

    viz.plot_reconstructions_normal(vae_model_2)   #synthetic data
    ...
    samples = viz.generate_samples(vae_model_2, n_tsne)
